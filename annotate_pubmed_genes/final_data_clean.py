...
and can optionally run a Gemini relevance analysis for each valid PMID/gene/disease pair.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.9,
        help="Minimum top_matches score to keep a row",
    )
    parser.add_argument(
        "--output_csv",
        required=True,
        help="Output CSV file with PMID, gene_symbol, and disease_domain",
    )
    parser.add_argument(
        "--debug",
        action="store_true",
    ...
    parser.add_argument(
        "--gemini_config",
        default=None,
        help="Optional Gemini config file. If provided, run Gemini analysis for each valid PMID/gene/disease pair.",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Resume from an existing output CSV by appending new rows and skipping already processed PMID/gene/disease rows.",
    )
...
def main() -> int:
    ...
    # Stream through parquet and build dicts:
    # pmid -> llm_gene_disease_map, pmid -> {(gene_symbol, disease_domain): score}
    pmid_to_llm = {}
    pmid_to_top_matches = {}
    pf = pq.ParquetFile(parquet_path)
    for batch in pf.iter_batches(columns=["pmid", "llm_gene_disease_map", "top_matches"]):
        pmids = batch.column(0).to_pylist()
        llm_vals = batch.column(1).to_pylist()
        top_match_vals = batch.column(2).to_pylist()
        for pmid, llm_val, top_match_val in zip(pmids, llm_vals, top_match_vals):
            pmid = str(pmid).strip()
            pmid_to_llm[pmid] = llm_val or []
            match_scores = {}
            for item in top_match_val or []:
                gene_symbol = str(item.get("gene_symbol", "")).strip().upper()
                disease_domain = str(item.get("disease_domain", "")).strip()
                score = item.get("score", None)
                if gene_symbol and disease_domain:
                    match_scores[(gene_symbol, disease_domain)] = score
            pmid_to_top_matches[pmid] = match_scores

    pmid_set = set(pmid_to_llm.keys())

    # Build gene2pubtator map: first column -> set of fourth column values.
    gene2pubtator_map = {}
    with gzip.open(gene2pubtator_path, "rt", encoding="utf-8") as f:
        for line in f:
            ...
            if key not in pmid_set:
                continue
            value = parts[3]
            gene_symbol = value.strip().upper()
            if gene_symbol:
                gene2pubtator_map.setdefault(key, set()).add(gene_symbol)

    processed_pairs = set()
    header = ["PMID", "gene_symbol", "disease_domain"]
    if gemini_enabled:
    ...
    if resume and output_path.exists():
        with output_path.open("r", newline="", encoding="utf-8") as existing_f:
            ...
            for row in reader:
                ...
                disease_domain = str(row.get("disease_domain", "")).strip()
                if pmid and gene_symbol and disease_domain:
                    processed_pairs.add((pmid, gene_symbol, disease_domain))
    ...
    with output_path.open(file_mode, newline="", encoding="utf-8") as out_f:
        ...
        for pmid in pmid_to_llm:
            llm_matches = pmid_to_llm[pmid] or []
            if not llm_matches:
                continue
            genes_for_pmid_set = gene2pubtator_map.get(pmid, set())

            if debug:
                print(f"\nPMID: {pmid}; genes: {sorted(genes_for_pmid_set)}")

            for match in llm_matches:
                total_count += 1
                gene_symbol = str(match.get("gene_symbol", "")).strip().upper()
                disease_domain = str(match.get("disease_domain", "")).strip()
                if not gene_symbol or not disease_domain:
                    continue

                # Filter by top_matches score
                if score_cutoff:
                    top_match_scores = pmid_to_top_matches.get(pmid, {})
                    score = top_match_scores.get((gene_symbol, disease_domain), None)
                    if score is not None and score < score_cutoff:
                        if debug:
                            print(
                                f"{pmid}\t{gene_symbol}\t{disease_domain}\t"
                                f"SCORE BELOW CUTOFF ({score:.2f} < {score_cutoff})"
                            )
                        continue

                if gene_symbol in genes_for_pmid_set:
                    valid_count += 1
                    pair = (str(pmid).strip(), gene_symbol, disease_domain)
                    if resume and pair in processed_pairs:
                        resume_skipped_count += 1
                        if debug:
                            print(f"{pmid}\t{gene_symbol}\t{disease_domain}\tSKIP RESUME")
                        continue
                    if debug:
                        print(f"{pmid}\t{gene_symbol}\t{disease_domain}\tVALID")
                    row = [pmid, gene_symbol, disease_domain]
                    if gemini_enabled:
                        relevance_label = ""
                        relevance_comment = ""
                        try:
                            print(
                                f"Gemini start: PMID {pmid} gene {gene_symbol} disease {disease_domain}",
                                flush=True,
                            )
                            ...
                            if article:
                                ...
                                print(
                                    f"Gemini done: PMID {pmid} gene {gene_symbol} disease {disease_domain} -> {relevance_label}",
                                    flush=True,
                                )
                            elif debug:
                                print(f"{pmid}\t{gene_symbol}\t{disease_domain}\tGemini skipped: article not found")
                            else:
                                print(
                                    f"Gemini skipped: PMID {pmid} gene {gene_symbol} disease {disease_domain} article not found",
                                    flush=True,
                                )
                        except Exception as exc:
                            print(
                                f"Gemini error for PMID {pmid} gene {gene_symbol} disease {disease_domain}: {exc}",
                                file=sys.stderr,
                            )
